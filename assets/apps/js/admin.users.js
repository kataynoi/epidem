$(document).ready(function(){
    ...
    users.ajax = {
        ...
        ,get_all_users_total: function (status,amp,hospcode, cb) {
            ...
            var url = '/users/save_edit_user_admin',
                params = {
                    items: items
                }
    ...
    users.get_user_edit=function(id){
        users.ajax.get_user(id, function (err, data) {
            if (err) {
                app.alert(err);
            } else {
                users.set_user_edit(data);
            }
        });
    }
    ...
    users.set_user_edit = function (data) {
        $('dd').empty();


        if (_.size(data.rows) > 0) {
            $('#edit_name').html(data.rows.name);
            $('#edit_email').html(data.rows.email);
            $('#edit_user_id').val(data.rows.id);
            $('#edit_office option[value="' + data.rows.office + '"]').prop('selected', true);
            $('#edit_user_level option[value="' + data.rows.sys_level + '"]').prop('selected', true);
    ...
    $('#btn_save_edit_user').on('click',function(){
        ...
        items.id = $('#edit_user_id').val();
        items.office = $('#edit_office').val();
        items.user_level = $('#edit_user_level').val();
        items.sys_id = $('#edit_sys_id').val();
