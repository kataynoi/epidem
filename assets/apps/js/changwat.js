$(function () {
    ...
    changwat.ajax = {
        ...
        },do_delete_e0: function (id, cb) {
            var url = '/changwat/do_delete_e0',
                params = {
                    id: id
                }
            ...
            _.each(data.rows, function (v) {
                ...
                $('#tbl_waiting_list > tbody').append(
                    '<tr ' + tr_death + '>' +
                        ...
                        '<a href="javascript:void(0);" class="btn btn-danger" data-id="' + v.id + '"' +
                        'data-name="btn_delete_waiting" ><i class="glyphicon glyphicon-trash"></i></a>' +
                        '</div></td>' +
                        '</tr>'
                );
                i++;
...
changwat.do_delete_e0=function(id){
    changwat.ajax.do_delete_e0(id, function(e) {
        if(e) {
            app.alert(e);
        } else {
    ...
    $('#btn_do_import').on('click', function () {
        ...
        if (data.length == 0) {
            ...
            if (confirm('คุณต้องการนำเข้ารายการทั้งหมด ' + data.length + ' รายการ ใช่หรือไม่?')) {
                changwat.ajax.do_import(data, function (err) {
                    if (err) {
                        app.alert(err);
                    }
                    else {
    ...
    $(document).on('click', 'a[data-name="btn_check_all"]', function(e) {
        e.preventDefault();
    ...
    $(document).on('click', 'a[data-name="btn_delete_waiting"]', function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        if(confirm('คุณต้องการลบผู้ป่วยรายนี้ใช่หรือไม่')){
            //alert('Del');
            changwat.do_delete_e0(id);
