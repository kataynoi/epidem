...
class Maps extends CI_Controller
{
        ...
        if($by_date)
        {
            ...
            if($rs)
            {
                ...
                foreach($rs as $r)
                {
                    ...
                    $obj->address   = get_address($r->addrcode);
    ...
    {
        $off_id=$this->session->userdata('hospcode');
        $rs = $this->maps->get_person_detail($id);
        $off_map=$this->maps->get_office_lat_lng($off_id);
        if($rs)
        {
            $data = array(
                'id'        => $id,
                ...
                'lat'       => $rs->latitude,
                'lng'       => $rs->longtitude,
                'office_lat' =>$off_map->latitude,
                'office_lng' =>$off_map->longtitude
            );
    ...
    public function set_map_office()
    {
        $hospcode=$this->hospcode;
        $rs = $this->maps->get_office_detail($hospcode);
        if($rs)
        {
            $data = array(
                'off_id'    => $rs->off_id,
                'off_name'  => $rs->off_name,
                'office_lat' =>$rs->latitude,
                'office_lng' =>$rs->longtitude
            );

            $this->layout->view('maps/set_map_office_view', $data);
        }
        else
        {
            show_404();
 ...
 public function save_map_office()
    {
        $off_id = $this->input->post('off_id');
        $lat = $this->input->post('lat');
        $lng = $this->input->post('lng');

        if(!empty($off_id) && !empty($lat) && !empty($lng))
        {
            $rs = $this->maps->save_map_office($off_id, $lat, $lng);
